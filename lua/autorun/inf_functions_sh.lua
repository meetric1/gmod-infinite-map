...
function InfMap.height_function(x, y) 
	if (x >= -0.01 and x < 1.51) and (y >= -0.01 and y < 1.51) then return -15 end
	//local final = InfMap.simplex.Noise3D(x / 10, y / 10, 100) / InfMap.simplex.Noise3D(x / 10, y / 10, 0) * 10000
	local holes =  1 / InfMap.simplex.Noise3D(x / 50, y / 50, 100)
    local final = holes + (InfMap.simplex.Noise3D(x / 10, y / 10, 10) * 100) * (InfMap.simplex.Noise3D(x / 100, y / 100, 40) + 0.31) * 2000
